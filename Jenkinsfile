pipeline {
    agent any
    environment{
        FULL_IMAGE = "445567113064.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}"
        TASK_DEFINITION =""
        NEW_TASK_DEFINITION=""
        NEW_TASK_INFO=""
        NEW_REVISION=""
    ...
    stages {
        stage('Checkout using tag') {
            steps{
                checkout([
                    ...
                    extensions: [], 
                    submoduleCfg: [], 
                    userRemoteConfigs: [[url: 'git@github.com:vagabond111vn/nodejs-random-color.git', credentialsId: 'github-key-udemy']]
                ])
            }
        ...
        stage('Upload image to ECR') {
            ...
            steps {
                sh 'aws ecr get-login-password --region ap-southeast-1 | docker login --username AWS --password-stdin 445567113064.dkr.ecr.ap-southeast-1.amazonaws.com'
                sh 'docker tag nodejs-random-color:${VERSION} 445567113064.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}'
                sh 'docker push 445567113064.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}'
            }
        }
